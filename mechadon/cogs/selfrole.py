...
class SelfroleCog(BaseCog):
    ...
    @commands.has_permissions(administrator=True)
    async def alias(
        self, context: Context, alias: str, *, role: RoleLenient = None
    ):
        alias_db = RoleLenient.get_alias(context, alias)
        if role is None:
            if not alias_db:
                raise CommandError(
                    f"Can not remove alias {alias} as it does not exist"
                )
    ...
    async def roles(self, context: Context):
        ...
        roles_db = (
            db.session.query(SelfRole)
            .filter(SelfRole.role_id.in_(role_ids))
            .all()
        )
        roles = filter(
