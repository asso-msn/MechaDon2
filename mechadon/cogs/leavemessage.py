...
class LeavemessageCog(BaseCog):
    ...
    def get_leave_message_channels(guild):
        return db.session.query(LeaveMessageChannel).filter_by(
            server_id=guild.id
        )
    ...
    async def leavechan(self, context: Context, *, channel: TextChannel = None):
        ...
        if entry := (
            db.session.get(LeaveMessageChannel, (channel.id, channel.guild.id))
        ):
            db.session.delete(entry)
            db.session.commit()
            await self.reply(
                context, f"Channel {channel} will no longer display leaves"
            )
            return
        entry = LeaveMessageChannel(
